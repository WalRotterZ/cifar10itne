...
class RP_GlobR:
    def __init__(self, NN_infos, dbg = False, bigM = None, timeout = None, lp_relax = False, nThreads = None, method=0):
        """
                ...
                if layer_info['activation'] == 'relu':
                    ...
                    for i in relax_nodes:
                        if suffix == '':
                            ...
                            ubi = max(dy_ub[i], 0)
                            diff = (ubi - lbi) if ubi - lbi > 0 else 1
                            if dy_lb[i] < 0 and dy_ub[i] > 0 and y_lb[i] > 0:
                                print("enter relax y_lb > 0 branch")
                                gp_model.addConstr(x[i] - x_orig[i] >= y[i] - y_orig[i],
                            ...
                            elif dy_lb[i] < 0 and dy_ub[i] > 0 and y_ub[i] < 0:
                                print("enter relax y_ub < 0 branch")
                                gp_model.addConstr(x[i] - x_orig[i] >= 0,
                                                   name='relu_lpRelax_1_dx_' + str(k) + '_' + str(i))
                                gp_model.addConstr(x[i] - x_orig[i] <= ubi * (y[i] - y_orig[i] - lbi) / diff,
                                                   name='relu_lpRelax_2_dx_' + str(k) + '_' + str(i))
                            else:
                                print("enter normal relax branch")
    ...
    def encoding_relu_output_constraint(self, gp_model,
        ...
        if lp_relax:
            ...
            y_out.ub = y_ub
            print("y_ub = "+str(y_ub))
            print("dy_lb = "+str(dy_lb))
            print("dy_ub = "+str(dy_ub))
            ...
            if self.method == 1:
                ...
                if dy_lb < 0 and dy_ub > 0 and y_lb > 0:
                    print("enter y_lb > 0 branch")
                    gp_model.addConstr(x_d_out - x_out >= y_d_out - y_out,
                                       name='relu_lpRelax_1_dx_out')
                    gp_model.addConstr(x_d_out - x_out <= ubi * (y_d_out - y_out - lbi) / diff,
                                       name='relu_lpRelax_2_dx_out')
                elif dy_lb < 0 and dy_ub > 0 and y_ub < 0:
                    print("enter y_ub < 0 branch")
                    gp_model.addConstr(x_d_out - x_out >= 0,
                                       name='relu_lpRelax_1_dx_out')
                    gp_model.addConstr(x_d_out - x_out <= ubi * (y_d_out - y_out - lbi) / diff,
                                       name='relu_lpRelax_2_dx_out')
                else:
                    print("enter normal branch")
                    gp_model.addConstr(x_d_out - x_out >= lbi * (ubi - (y_d_out - y_out)) / diff,
                                       name='relu_lpRelax_1_dx_out')
                    gp_model.addConstr(x_d_out - x_out <= ubi * (y_d_out - y_out - lbi) / diff,
                                       name='relu_lpRelax_2_dx_out')
    ...
    def range_propergation_3(self, input_lb, input_ub, diff_lb, diff_ub, window_size = 1, reset_bound = True):
        ...
        l_in = []
        u_in = []
        with open('vnnlib/cifar10_spec_idx_17_eps_0.00784_n1.vnnlib', 'r') as file:
            for line in file:
                ...
                if upper_match:
                    ...
                    u_in.append(value)
        assert len(l_in) == len(u_in) == len(input_lb)
        for k in range(len(l_in)):
            input_lb[k] = l_in[k]
            input_ub[k] = u_in[k]
        '''
...
def load_vnnlib(filename, magnification=2):
    lower_bound_pattern = re.compile(r'\(assert \(>= X_(\d+) ([\d\.eE+-]+)\)\)')
    upper_bound_pattern = re.compile(r'\(assert \(<= X_(\d+) ([\d\.eE+-]+)\)\)')
    lower_bounds, upper_bounds = [], []
    r_lower, r_upper = [], []
    vnnlib_name = os.path.splitext(os.path.basename(filename))[0]
    ...
    for lower, upper in zip(lower_bounds, upper_bounds):
        ...
        r_lower.append(new_lower)
        r_upper.append(new_upper)
    return r_lower, r_upper


def load_cifar10_config(config_file):
    ...
    while i < len(lines):
        if len(lines[i].split()) < 2:
            raise ValueError(f"The configuration file format is incorrect, the content of line {i + 1}:{lines[i]}")
        model_name, num_properties = lines[i].split()
        num_properties = int(num_properties)
        if i + num_properties + 1 > len(lines):
            raise ValueError(f"The number of properties in the configuration file does not match the number of rows, model name:{model_name}")
        vnnlib_files = [lines[i + j + 1] for j in range(num_properties)]
        config_data.append((model_name, vnnlib_files))
        i += num_properties + 1
    return config_data


config_file = 'cifar10_config.txt'
configs = load_cifar10_config(config_file)

for model_name, vnnlib_files in configs:

    json_filename = 'data/model/' + model_name + '.json'
    ...
    dnn_model.load_weights('data/model/' + model_name + '.h5')


    layers = nn_extractor(dnn_model)
    print(layers)

    for layer in layers:
        if layer['type'] == 'normalization':
            layer['mean'] = layer['mean'].reshape((32, 32, 3))
            layer['std'] = layer['std'].reshape((32, 32, 3))
            layer['dim'] = len(layer['mean'].shape)
            layer['input_shape'] = layer['mean'].shape
            layer['output_shape'] = layer['mean'].shape


    for vnnlib_name in vnnlib_files:
        vnnlib_file = 'cifar10vnnlib/' + vnnlib_name + '.vnnlib'
        lower_bounds, upper_bounds = load_vnnlib(vnnlib_file)


        g0 = RP_GlobR(NN_infos=layers, lp_relax=True, method=0)
        g1 = RP_GlobR(NN_infos=layers, lp_relax=True, method=1)
        eps = 0.001
        input_lb = np.array(lower_bounds).reshape((32, 32, 3))
        input_ub = np.array(upper_bounds).reshape((32, 32, 3))

        diff_lb = -eps * np.ones_like(input_lb)
        diff_ub = eps * np.ones_like(input_lb)

        start_time0 = time.time()
        out_bound0 = g0.range_propergation_3(input_lb=input_lb, input_ub=input_ub, diff_lb=diff_lb, diff_ub=diff_ub)
        end_time0 = time.time()

        result_filename0 = f'cifar10result/{model_name}_{vnnlib_name}_method0.txt'
        with open(result_filename0, 'w') as result_file0:
            result_file0.write(str(out_bound0))

        execution_time0 = end_time0 - start_time0
        execution_time_filename0 = f'cifar10result/{model_name}_{vnnlib_name}_method0_time.txt'
        with open(execution_time_filename0, 'w') as time_file0:
            time_file0.write(f"Execution time: {execution_time0} seconds")

        start_time1 = time.time()
        out_bound1 = g1.range_propergation_3(input_lb=input_lb, input_ub=input_ub, diff_lb=diff_lb, diff_ub=diff_ub)
        end_time1 = time.time()

        result_filename1 = f'cifar10result/{model_name}_{vnnlib_name}_method1.txt'
        with open(result_filename1, 'w') as result_file1:
            result_file1.write(str(out_bound1))

        execution_time1 = end_time1 - start_time1
        execution_time_filename1 = f'cifar10result/{model_name}_{vnnlib_name}_method1_time.txt'
        with open(execution_time_filename1, 'w') as time_file1:
            time_file1.write(f"Execution time: {execution_time1} seconds")

'''

netname = 'model_cifar10_4c3d_noBN_reg_1e_3_0'
...
for layer in layers:
    if layer['type'] == 'normalization':
        print(layer['mean'].shape)
        print(layer['std'].shape)
        layer['mean'] = layer['mean'].reshape((32,32,3))
        layer['std'] = layer['std'].reshape((32,32,3))
        print(layer['mean'].shape)
        print(layer['std'].shape)
...
lower_bounds, upper_bounds = load_vnnlib("vnnlib/cifar10_spec_idx_0_eps_0.00784_n1.vnnlib")

g = RP_GlobR(NN_infos=layers,lp_relax=True)
eps = 0.001
#input_lb = np.zeros((32,32,3))
#input_ub = np.ones((32,32,3))
input_lb = np.array(lower_bounds).reshape((32, 32, 3))
input_ub = np.array(upper_bounds).reshape((32, 32, 3))
...
out_bound = g.range_propergation_3(input_lb=input_lb,input_ub=input_ub,diff_lb=diff_lb,diff_ub=diff_ub)
end_time = time.time()
print(out_bound)
execution_time = end_time - start_time
print(f"range_propergation_3 execution time: {execution_time} seconds")
